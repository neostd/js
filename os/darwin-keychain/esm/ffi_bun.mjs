...
function ptrToBytes(intPtr, len) {
  ...
  for (let i = 0; i < len; i++) out[i] = read.u8(intPtr, i);
  return out;
}
...
function releaseFindResult(found) {
  if (!found) return;
  if (found.dataPtr) sec.symbols.SecKeychainItemFreeContent(null, found.dataPtr);
  if (found.itemPtr) cf.symbols.CFRelease(found.itemPtr);
}
const backend = {
