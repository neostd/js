...
const koffi = createRequire(import.meta.url ?? "file:///")("koffi");
...
function readPtr(address, offset) {
  ...
  const ptr = new DataView(new Uint8Array(bytes).buffer).getBigUint64(0, true);
...
function getAccountAttribute(itemPtr) {
  ...
  const attrsOut = [null];
  if (
    SecKeychainItemCopyAttributesAndData(itemPtr, info, null, attrsOut, [0], null) !== 0 ||
    !attrsOut[0]
  )
    return "";
  try {
    ...
    if (readU32(attrsArrayAddress, 0) !== ATTR_ACCOUNT) return "";
...
const backend = {
  ...
  list(service) {
    ...
    try {
      while (true) {
        const itemOut = [null];
        if (SecKeychainSearchCopyNext(searchOut[0], itemOut) !== 0 || !itemOut[0]) break;
        try {
