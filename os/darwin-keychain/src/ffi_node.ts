...
function ptrToBytes(ptr: bigint, length: number): Uint8Array {
  return new Uint8Array(ffi.toArrayBuffer(ptr, length));
}

function readU32(ptr: bigint, offset: number): number {
  return ffi.getUint32(ptr, offset);
}

function readU64(ptr: bigint, offset: number): bigint {
  return ffi.getUint64(ptr, offset);
}

function makeServiceAttrList(service: string): {
  attrs: Uint8Array;
  list: Uint8Array;
  refs: Uint8Array[];
} {
...
function getAccountAttribute(itemPtr: bigint): string {
  ...
  try {
    ...
    if (attrTag !== ATTR_ACCOUNT) return "";
