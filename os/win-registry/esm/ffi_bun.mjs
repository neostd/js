import { stringToWide, wideToString } from "./types.mjs";
...
function readU64(buf, offset = 0) {
  ...
  return (BigInt(readU32(buf, offset + 4)) << 32n) | lo;
}
const backend = {
  ...
  deleteKey(hkey, subKey) {
    return symbols.RegDeleteKeyW(hkeyToPtr(hkey), ptr(stringToWide(subKey)));
  },
  deleteValue(hkey, valueName) {
    return symbols.RegDeleteValueW(hkeyToPtr(hkey), ptr(stringToWide(valueName)));
  },
  queryInfoKey(hkey) {
    ...
    if (status !== 0) throw new Error(`RegQueryInfoKeyW failed with error code ${status}`);
    return {
  ...
  enumKeyNames(hkey, index, nameBufferSize) {
    ...
    if (status !== 0) throw new Error(`RegEnumKeyExW failed with error code ${status}`);
  ...
  enumValueNames(hkey, index, nameBufferSize) {
    ...
    if (status === 259) return null;
    if (status !== 0) throw new Error(`RegEnumValueW failed with error code ${status}`);
    return wideToString(nameBuf, readU32(sizeBuf) * 2);
  },
  ...
  setValue(hkey, valueName, type, data) {
    ...
    if (status !== 0) throw new Error(`RegSetValueExW failed with error code ${status}`);
  },
};
//#endregion
export { backend };
