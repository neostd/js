...
const backend = {
  write(cred, flags) {
    ...
    if (!lib.functions.CredWriteW(structBuf, flags))
      throw new Error(`CredWriteW failed with error code ${k32.functions.GetLastError()}`);
  },
  read(targetName, type) {
    const wTarget = stringToWide(targetName);
    const outBuf = new Uint8Array(8);
    if (!lib.functions.CredReadW(wTarget, type, 0, outBuf)) return null;
