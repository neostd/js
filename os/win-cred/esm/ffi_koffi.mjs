...
function toKoffiCredential(cred) {
  const low = Number(cred.lastWritten & 4294967295n);
  const high = Number((cred.lastWritten >> 32n) & 4294967295n);
  return {
    Flags: cred.flags,
    Type: cred.type,
...
const backend = {
  ...
  enumerate(filter, flags) {
    const countArr = [0];
    const credsArr = [null];
    if (!CredEnumerateW(filter, flags, countArr, credsArr)) return [];
    ...
    try {
      ...
      if (credsArr[0]) CredFree(credsArr[0]);
    }
    return results;
  },
};
