import { stringToWide } from "./types.mjs";
...
function buildCredentialBuffer(cred) {
  const refs = [];
...
const backend = {
  write(cred, flags) {
    ...
    if (!symbols.CredWriteW(ptr(structBuf), flags))
      throw new Error(`CredWriteW failed with error code ${k32s.GetLastError()}`);
  },
  read(targetName, type) {
    const wTarget = stringToWide(targetName);
    const outBuf = new Uint8Array(8);
    if (!symbols.CredReadW(ptr(wTarget), type, 0, ptr(outBuf))) return null;
  ...
  enumerate(filter, flags) {
    ...
    try {
      for (let i = 0; i < count; i++) {
        const credPtr = Number(read.ptr(arrayPtr, i * 8));
