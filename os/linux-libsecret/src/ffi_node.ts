import type { LinuxKeyringBackend, SecretRecord } from "./types.ts";
import process from "node:process";

const { createRequire } = process.getBuiltinModule("node:module");
const require = createRequire(import.meta.url ?? "file:///");
const ffi = require("node:ffi");

const libsecret = ffi.dlopen("libsecret-1.so.0", {
  secret_schema_new: {
    parameters: ["pointer", "u32", "pointer", "u32", "pointer", "u32", "pointer"],
    result: "pointer",
  },
  secret_password_lookup_sync: {
    parameters: [
      "pointer",
      "pointer",
      "pointer",
      "pointer",
      "pointer",
      "pointer",
      "pointer",
      "pointer",
    ],
    result: "pointer",
  },
  secret_password_store_sync: {
    parameters: [
      "pointer",
      "pointer",
      "pointer",
      "pointer",
      "pointer",
      "pointer",
      "pointer",
      "pointer",
      "pointer",
      "pointer",
      "pointer",
    ],
    result: "u8",
  },
  secret_password_clear_sync: {
    parameters: [
      "pointer",
      "pointer",
      "pointer",
      "pointer",
      "pointer",
      "pointer",
      "pointer",
      "pointer",
    ],
    result: "u8",
  },
  secret_password_search_sync: {
    parameters: ["pointer", "u32", "pointer", "pointer", "pointer", "pointer", "pointer"],
    result: "pointer",
  },
  secret_retrievable_get_attributes: {
    parameters: ["pointer"],
    result: "pointer",
  },
  secret_retrievable_retrieve_secret_sync: {
    parameters: ["pointer", "pointer", "pointer"],
    result: "pointer",
  },
  secret_value_get: {
    parameters: ["pointer", "pointer"],
    result: "pointer",
  },
  secret_value_unref: {
    parameters: ["pointer"],
    result: "void",
  },
  secret_password_free: {
    parameters: ["pointer"],
    result: "void",
  },
});

const glib = ffi.dlopen("libglib-2.0.so.0", {
  g_error_free: {
    parameters: ["pointer"],
    result: "void",
  },
  g_hash_table_lookup: {
    parameters: ["pointer", "pointer"],
    result: "pointer",
  },
  g_hash_table_unref: {
    parameters: ["pointer"],
    result: "void",
  },
  g_list_free: {
    parameters: ["pointer"],
    result: "void",
  },
});

const gobject = ffi.dlopen("libgobject-2.0.so.0", {
  g_object_unref: {
    parameters: ["pointer"],
    result: "void",
  },
});

const enc = new TextEncoder();
const dec = new TextDecoder();

let schemaPtr: bigint | null = null;

function cstr(value: string): Uint8Array {
  const b = enc.encode(value);
  const out = new Uint8Array(b.length + 1);
  out.set(b);
  return out;
}

function readCString(ptr: bigint): Uint8Array {
  const bytes: number[] = [];
  for (let i = 0; ; i++) {
    const b = ffi.getUint8(ptr, i);
    if (b === 0) break;
    bytes.push(b);
  }
  return new Uint8Array(bytes);
}

function readCStringText(ptr: bigint): string {
  return dec.decode(readCString(ptr));
}

function listDataPtr(listPtr: bigint): bigint {
  return ffi.getUint64(listPtr, 0);
}

function listNextPtr(listPtr: bigint): bigint {
  return ffi.getUint64(listPtr, 8);
}

function throwIfError(errorOut: Uint8Array): void {
  const errPtr = new DataView(errorOut.buffer).getBigUint64(0, true);
  if (!errPtr) return;
  const msgPtr = ffi.getUint64(errPtr, 8);
  const msg = msgPtr ? dec.decode(readCString(msgPtr)) : "Unknown libsecret error";
  glib.functions.g_error_free(errPtr);
  throw new Error(msg);
}

function getSchema(): bigint {
  if (schemaPtr !== null) return schemaPtr;
  const schema = libsecret.functions.secret_schema_new(
    cstr("org.freedesktop.Secret.Generic"),
    0,
    cstr("service"),
    0,
    cstr("account"),
    0,
    null,
  );
  if (schema === null) {
    throw new Error("Failed to create libsecret schema");
  }
  schemaPtr = schema;
  return schema;
}

export const backend: LinuxKeyringBackend = {
  getSecretBytes(service: string, account: string): Uint8Array | null {
    const errorOut = new Uint8Array(8);
    const p = libsecret.functions.secret_password_lookup_sync(
      getSchema(),
      null,
      errorOut,
      cstr("service"),
      cstr(service),
      cstr("account"),
      cstr(account),
      null,
    );
    throwIfError(errorOut);
    if (!p) return null;
    try {
      return readCString(p);
    } finally {
      libsecret.functions.secret_password_free(p);
    }
  },

  setSecretBytes(service: string, account: string, secret: Uint8Array): void {
    const errorOut = new Uint8Array(8);
    const ok = libsecret.functions.secret_password_store_sync(
      getSchema(),
      cstr("default"),
      cstr(`${service}/${account}`),
      cstr(dec.decode(secret)),
      null,
      errorOut,
      cstr("service"),
      cstr(service),
      cstr("account"),
      cstr(account),
      null,
    );
    throwIfError(errorOut);
    if (!ok) throw new Error("Failed to store secret");
  },

  deleteSecret(service: string, account: string): boolean {
    const errorOut = new Uint8Array(8);
    const ok = libsecret.functions.secret_password_clear_sync(
      getSchema(),
      null,
      errorOut,
      cstr("service"),
      cstr(service),
      cstr("account"),
      cstr(account),
      null,
    );
    throwIfError(errorOut);
    return !!ok;
  },

  list(serviceName: string): SecretRecord[] {
    const errorOut = new Uint8Array(8);
    const list = libsecret.functions.secret_password_search_sync(
      getSchema(),
      2,
      null,
      errorOut,
      cstr("service"),
      cstr(serviceName),
      null,
    );
    throwIfError(errorOut);
    if (!list) return [];

    const results: SecretRecord[] = [];
    try {
      for (let node = list; node; node = listNextPtr(node)) {
        const retrievable = listDataPtr(node);
        if (!retrievable) continue;

        const attributes = libsecret.functions.secret_retrievable_get_attributes(retrievable);
        try {
          const accountPtr = glib.functions.g_hash_table_lookup(attributes, cstr("account"));
          if (!accountPtr) continue;

          const secretErrorOut = new Uint8Array(8);
          const value = libsecret.functions.secret_retrievable_retrieve_secret_sync(
            retrievable,
            null,
            secretErrorOut,
          );
          throwIfError(secretErrorOut);
          if (!value) continue;

          try {
            const lengthOut = new Uint8Array(8);
            const secretPtr = libsecret.functions.secret_value_get(value, lengthOut);
            if (!secretPtr) continue;
            results.push({
              service: serviceName,
              account: readCStringText(accountPtr),
              secret: readCStringText(secretPtr),
            });
          } finally {
            libsecret.functions.secret_value_unref(value);
          }
        } finally {
          glib.functions.g_hash_table_unref(attributes);
        }
      }
    } finally {
      for (let node = list; node; node = listNextPtr(node)) {
        const retrievable = listDataPtr(node);
        if (retrievable) gobject.functions.g_object_unref(retrievable);
      }
      glib.functions.g_list_free(list);
    }

    return results;
  },
};
