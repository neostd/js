...
let schemaPtr: number | null = null;
...
function keep(value: Uint8Array): Pointer {
  refs.push(value);
  return ptr(value);
}

function readErrorMessage(errorPtr: number): string {
  const msgPtr = Number(read.ptr(errorPtr as Pointer, 8));
  if (!msgPtr) return "Unknown libsecret error";
...
function throwIfError(errorOut: Uint8Array): void {
  ...
  glib.symbols.g_error_free(errPtr as Pointer);
  throw new Error(msg);
}
...
function getSchema(): number {
  ...
  schemaPtr = Number(
    libsecret.symbols.secret_schema_new(
      keep(cstr("org.freedesktop.Secret.Generic")),
      0,
      keep(cstr("service")),
      ATTR_STRING,
      keep(cstr("account")),
...
export const backend: LinuxKeyringBackend = {
  getSecretBytes(service: string, account: string): Uint8Array | null {
    ...
    const p = Number(
      libsecret.symbols.secret_password_lookup_sync(
        getSchema() as Pointer,
        null,
        ptr(errorOut),
        ptr(cstr("service")),
        ptr(cstr(service)),
    ...
    try {
      ...
      libsecret.symbols.secret_password_free(p as Pointer);
    }
  },
  ...
  setSecretBytes(service: string, account: string, secret: Uint8Array): void {
    ...
    const ok = libsecret.symbols.secret_password_store_sync(
      getSchema() as Pointer,
      ptr(cstr("default")),
      ptr(cstr(`${service}/${account}`)),
      ptr(cstr(dec.decode(secret))),
      null,
      ptr(errorOut),
      ptr(cstr("service")),
      ptr(cstr(service)),
  ...
  list(_serviceName: string): SecretRecord[] {
    throw new Error(
      "linux-keyring list is not supported on Bun because Bun FFI crashes when reading libsecret retrievable results",
    );
  },
};
