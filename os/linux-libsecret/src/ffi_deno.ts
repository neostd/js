...
function listDataPtr(listPtr: bigint): bigint {
  const view = new Deno_.UnsafePointerView(Deno_.UnsafePointer.create(listPtr));
...
function readU64(view: InstanceType<typeof Deno_.UnsafePointerView>, offset: number): bigint {
  const lo =
...
function readErrorMessage(errorPtr: bigint): string {
  ...
  const msg = new Deno_.UnsafePointerView(Deno_.UnsafePointer.create(msgPtr)).getCString();
