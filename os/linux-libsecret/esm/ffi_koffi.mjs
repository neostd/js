...
function ptrAt(ptrValue, offset) {
...
const backend = {
  ...
  list(serviceName) {
    ...
    const results = [];
    try {
      for (let node = list; node; node = ptrAt(node, 8)) {
      ...
      for (let node = list; node; node = ptrAt(node, 8)) {
        const retrievable = ptrAt(node, 0);
        if (retrievable) g_object_unref(retrievable);
      }
      g_list_free(list);
    }
    return results;
