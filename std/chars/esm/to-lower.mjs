import { r as CaseRanges } from "./case-BgNeCTGR.mjs";
import "./constants.mjs";
//#region src/to-lower.ts
/**
 ...
 * @example
 * ```typescript
 * import { toLower } from '@neostd/chars';
 *
...
function toLower(char) {
  if (char < 128) {
    if (char >= 65 && char <= 90) return char + 32;
    return char;
  }
  let lo = 0;
  ...
  while (lo < hi) {
    ...
    if (l <= char && char <= h) {
      const delta = d[1];
      if (delta > 1114111) return ((l + (char - l)) & -2) | 1;
      return char + delta;
    }
    if (char < l) hi = mid;
    else lo = mid + 1;
