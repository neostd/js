...
import { Nd } from "./tables/nd.mjs";
//#region src/is_digit.ts
/**
...
function isDigit(char) {
	...
	if (char < 256) return (latin1[char] & 4) !== 0;
	if (char <= Nd.R16[Nd.R32.length - 1][1]) return is16(Nd.R16, char);
	if (char >= Nd.R32[0][0]) return is32(Nd.R32, char);
	return false;
}
/**
...
* ```
*/
function isDigitAt(value, index) {
	return isDigitUnsafe(value.codePointAt(index) ?? 0);
}
//#endregion
