...
function coerceValue(name: string, value: string | boolean, options: ParseOptions): PrimitiveValue {
  ...
  if (hasName(name, options.string)) {
    return value;
  }
...
export function parse(
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg.startsWith("--no-") && !arg.includes("=")) {
      ...
      setValue(result, name, false, aliases);
      continue;
    }
    ...
    if (arg.startsWith("--")) {
      ...
      if (hasName(name, options.boolean) || args[i + 1] === undefined || isOption(args[i + 1])) {
    ...
    if (hasName(short, options.boolean) || args[i + 1] === undefined || isOption(args[i + 1])) {
    ...
    if (result[key] === undefined) {
      if (Array.isArray(value)) {
        for (const item of value) {
          setValue(result, key, item, aliases);
        }
      } else {
