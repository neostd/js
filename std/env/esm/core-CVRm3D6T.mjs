...
import { a as globals, i as WINDOWS, r as BROWSER, t as expand$1 } from "./expand-6F8favQH.mjs";
//#region src/core.ts
...
const proxy = new Proxy(
  {},
  {
    ...
    has(_target, property) {
      return typeof property === "string" && property.length > 0 && getEnv(property) !== void 0;
    },
    ownKeys() {
...
function merge(values) {
  for (const [key, value] of Object.entries(values))
    if (value === void 0 || value === null) delete proxy[key];
...
function union(values) {
  for (const [key, value] of Object.entries(values))
    if (value !== void 0 && proxy[key] === void 0) proxy[key] = value;
}
/**
 ...
 * ```
 */
function hasPath(value, paths = splitPath()) {
 ...
 * ```
 */
function appendPath(path, force = false) {
 ...
 * ```
 */
function prependPath(path, force = false) {
  const paths = splitPath();
  if (force || !hasPath(path, paths)) {
    paths.unshift(path);
    setPath(joinPath(paths));
