...
import { CHAR_SPACE, CHAR_UNDERSCORE } from "@neostd/chars/constants";
...
import { isUpper } from "@neostd/chars/is-upper";
 ...
 * ```
 */
const NoCapitalizeWords = new Tokens();
[
  "and",
  "or",
  "nor",
 ...
 */
function titleize(s) {
  if (typeof s === "string") s = toCharSliceLike(s);
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    if (c === CHAR_UNDERSCORE || isSpace(c)) {
      ...
      if (last === CHAR_UNDERSCORE) continue;
      tokens.push(sb.toArray());
      sb.clear();
      last = c;
      continue;
  ...
  for (const token of tokens) {
    ...
    for (const title of NoCapitalizeWords)
      if (equalFold(title, token)) {
        if (sb.length > 0) sb.appendChar(CHAR_SPACE);
        sb.appendCharArray(title);
        skip = true;
        break;
      }
    if (skip) continue;
