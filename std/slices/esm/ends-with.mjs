 ...
 * ```ts
 * import { endsWith } from "@neostd/slices/ends-with";
 *
 ...
 * ```ts
 * import { endsWithFold } from "@neostd/slices/ends-with";
 *
 ...
 * ```
 */
function endsWithFold(value, test) {
 ...
 * ```
 */
function endsWith(value, test) {
  const s = toCharSliceLike(value);
  const t = toCharSliceLike(test);
  if (t.length > s.length) return false;
  for (let i = 0; i < t.length; i++) if (s.at(s.length - t.length + i) !== t.at(i)) return false;
  return true;
}
//#endregion
