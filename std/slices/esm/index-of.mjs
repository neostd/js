...
import { simpleFold } from "@neostd/chars/simple-fold";
 ...
 * ```ts
 * import { indexOf } from "@neostd/slices/index-of";
 *
 ...
 * ```ts
 * import { indexOfFold } from "@neostd/slices/index-of";
 *
 ...
 * ```
 */
function indexOfFold(value, test, index = 0) {
  ...
  if (t.length == 0 || s.length == 0 || t.length > s.length) return -1;
  let f = 0;
  let i = index;
  for (; i < s.length; i++) {
    let sr = s.at(i) ?? -1;
    ...
    if ((sr | tr) >= 128) {
      ...
      for (; j < s.length; j++) {
        ...
        if (tr === sr) {
          f++;
          if (f === t.length) return j - t.length + 1;
          continue;
        }
        if (tr < sr) {
          const tmp = tr;
      ...
      if (f === t.length) return j - t.length;
      return -1;
    }
    if (tr === sr) {
      f++;
      if (f === t.length) return i + 1 - f;
      continue;
    }
    if (tr < sr) {
      const tmp = tr;
 ...
 * ```
 */
function indexOf(value, test, index = 0) {
  const s = toCharSliceLike(value);
  const t = toCharSliceLike(test);
  if (t.length == 0 || s.length == 0 || t.length > s.length) return -1;
  let f = 0;
  ...
  for (; i < s.length; i++) {
    ...
    if (sr === tr) {
      f++;
      if (f === t.length) return i - t.length + 1;
      continue;
    }
    f = 0;
  }
