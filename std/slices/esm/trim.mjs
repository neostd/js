 ...
 * @example
 * ```typescript
 * import { trim, trimStart, trimEnd } from '@neostd/slices/trim';
 *
 ...
 * @example
 * ```typescript
 * import { trimEndSpace } from '@neostd/slices/trim';
 *
 ...
 * ```
 */
function trimEndSpace(buffer) {
  const s = toCharSliceLike(buffer);
  let size = s.length;
  for (let i = s.length - 1; i >= 0; i--)
    if (isSpace(s.at(i) ?? -1)) size--;
    else break;
  ...
  for (let i = 0; i < size; i++) buffer2[i] = s.at(i) ?? 0;
  return buffer2;
}
/**
 ...
 * @example
 * ```typescript
 * import { trimEndChar } from '@neostd/slices/trim';
 *
 * String.fromCodePoint(...trimEndChar("hello...", 46)); // "hello" (46 = '.')
 * String.fromCodePoint(...trimEndChar("hello", 46)); // "hello" (no change)
 * ```
 */
function trimEndChar(buffer, suffix) {
  const s = toCharSliceLike(buffer);
  let size = s.length;
  for (let i = s.length - 1; i >= 0; i--)
    if (s.at(i) === suffix) size--;
    else break;
  if (size === s.length && s instanceof Uint32Array) return s;
  if (s instanceof Uint32Array) return s.subarray(0, size);
  const buffer2 = new Uint32Array(size);
  for (let i = 0; i < size; i++) buffer2[i] = s.at(i) ?? 0;
  return buffer2;
}
 ...
 * @example
 * ```typescript
 * import { trimEndSlice } from '@neostd/slices/trim';
 *
 ...
 * ```
 */
function trimEndSlice(buffer, suffix) {
  const s = toCharSliceLike(buffer);
 ...
 * @example
 * ```typescript
 * import { trimEnd } from '@neostd/slices/trim';
 *
 ...
 * ```
 */
function trimEnd(buffer, suffix) {
  if (suffix === void 0) return trimEndSpace(buffer);
  if (suffix.length === 1) return trimEndChar(buffer, toCharSliceLike(suffix).at(0) ?? -1);
 ...
 * @example
 * ```typescript
 * import { trimStartSpace } from '@neostd/slices/trim';
 *
 ...
 * ```
 */
function trimStartSpace(buffer) {
  const s = toCharSliceLike(buffer);
  let size = s.length;
  ...
  if (s instanceof Uint32Array) return s.subarray(offset);
  const buffer2 = new Uint32Array(size);
  for (let i = 0; i < size; i++) buffer2[i] = s.at(offset + i) ?? 0;
  return buffer2;
}
/**
 ...
 * @example
 * ```typescript
 * import { trimStartChar } from '@neostd/slices/trim';
 *
 * String.fromCodePoint(...trimStartChar("...hello", 46)); // "hello" (46 = '.')
 * String.fromCodePoint(...trimStartChar("hello", 46)); // "hello" (no change)
 * ```
 */
function trimStartChar(buffer, prefix) {
  if (!Number.isInteger(prefix) || prefix < 0 || prefix > 1114111)
  ...
  if (size === s.length && s instanceof Uint32Array) return s;
  if (s instanceof Uint32Array) return s.subarray(start);
  const buffer2 = new Uint32Array(size);
  for (let j = start, i = 0; j < s.length; j++, i++) buffer2[i] = s.at(j) ?? 0;
  return buffer2;
}
/**
 ...
 * @example
 * ```typescript
 * import { trimStartSlice } from '@neostd/slices/trim';
 *
 ...
 * ```
 */
function trimStartSlice(buffer, prefix) {
  const s = toCharSliceLike(buffer);
  ...
  if (size === s.length && s instanceof Uint32Array) return s;
  if (s instanceof Uint32Array) return s.subarray(j);
  const buffer2 = new Uint32Array(size);
  for (let i = 0; i < size; i++) buffer2[i] = s.at(i + j) ?? 0;
  return buffer2;
}
/**
 ...
 * @example
 * ```typescript
 * import { trimStart } from '@neostd/slices/trim';
 *
 * // Remove whitespace (default)
 * String.fromCodePoint(...trimStart("  hello")); // "hello"
 *
 ...
 * ```
 */
function trimStart(buffer, prefix) {
  if (prefix === void 0) return trimStartSpace(buffer);
  if (prefix.length === 1) return trimStartChar(buffer, toCharSliceLike(prefix).at(0) ?? -1);
 ...
 * @example
 * ```typescript
 * import { trimSpace } from '@neostd/slices/trim';
 *
 ...
 * ```
 */
function trimSpace(buffer) {
  const s = toCharSliceLike(buffer);
  let start = 0;
  let end = s.length;
  for (let i = 0; i < s.length; i++)
    if (isSpace(s.at(i) ?? -1)) start++;
    else break;
  if (start === s.length) return new Uint32Array(0);
  for (let i = s.length - 1; i >= 0; i--)
    if (isSpace(s.at(i) ?? -1)) end--;
    else break;
  if (start === 0 && end === s.length && s instanceof Uint32Array) return s;
  if (s instanceof Uint32Array) return s.subarray(start, end);
  const buffer2 = new Uint32Array(end - start);
  for (let i = start; i < end; i++) buffer2[i - start] = s.at(i) ?? 0;
  return buffer2;
}
/**
...
function trimChar(buffer, prefix) {
  ...
  let end = s.length;
  for (let i = 0; i < s.length; i++)
    if (s.at(i) === prefix) start++;
    else break;
  if (start === s.length) return new Uint32Array(0);
  for (let i = s.length - 1; i >= 0; i--)
    if (s.at(i) === prefix) end--;
    else break;
  if (start === 0 && end === s.length && s instanceof Uint32Array) return s;
  if (s instanceof Uint32Array) return s.subarray(start, end);
  const buffer2 = new Uint32Array(end - start);
 ...
 * @example
 * ```typescript
 * import { trimSlice } from '@neostd/slices/trim';
 *
 ...
 * ```
 */
function trimSlice(buffer, chars) {
  const s = toCharSliceLike(buffer);
 ...
 * @example
 * ```typescript
 * import { trim } from '@neostd/slices/trim';
 *
 * // Remove whitespace (default)
 ...
 * ```
 */
function trim(buffer, chars) {
  if (chars === void 0) return trimSpace(buffer);
  if (chars.length === 1) return trimChar(buffer, toCharSliceLike(chars).at(0) ?? -1);
