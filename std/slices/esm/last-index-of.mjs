 ...
 * ```ts
 * import { lastIndexOf } from "@neostd/slices/last-index-of";
 *
 ...
 * ```ts
 * import { lastIndexOfFold } from "@neostd/slices/last-index-of";
 *
 ...
 * ```
 */
function lastIndexOfFold(value, test, index = 0) {
 ...
 * ```
 */
function lastIndexOf(value, test, index = Infinity) {
  const s = toCharSliceLike(value);
  const t = toCharSliceLike(test);
  if (t.length === 0 || s.length === 0 || t.length > s.length) return -1;
  const l = Math.min(s.length, index === Infinity ? s.length : index + 1);
