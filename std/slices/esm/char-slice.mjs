import { toCharArray, toCharSliceLike } from "./utils.mjs";
...
import { isSpace, toLower, toUpper } from "@neostd/chars";
//#region src/char-slice.ts
/**
 ...
 */
var ReadonlyCharSlice = class ReadonlyCharSlice {
  #buffer;
  #start;
  #end;
  /**
   ...
   * @example
   * ```typescript
   * import { ReadOnlyCharSlice } from '@neostd/slices/char-slice';
   *
   ...
   */
  static fromString(s) {
    return new ReadonlyCharSlice(toCharArray(s));
  }
  /**
   ...
   */
  forEach(callback) {
    for (let i = 0; i < this.length; i++) callback(this.#buffer[this.#start + i], i);
    return this;
  }
  /**
  ...
  map(callback) {
    ...
    for (let i = 0; i < this.length; i++) buffer[i] = callback(this.#buffer[this.#start + i], i);
  ...
  indexOf(value, index = 0) {
    if (typeof value === "number") value = new Uint32Array([value]);
  ...
  get toLower() {
    const buffer = new Uint32Array(this.length);
    let i = 0;
    for (let j = this.#start; j < this.#end; j++) buffer[i++] = toLower(this.#buffer[j]);
    return new ReadonlyCharSlice(buffer);
  }
  /**
  ...
  get toUpper() {
    const buffer = new Uint32Array(this.length);
    let i = 0;
    for (let j = this.#start; j < this.#end; j++) buffer[i++] = toUpper(this.#buffer[j]);
    return new ReadonlyCharSlice(buffer);
  }
  /**
  ...
  trimStartSpace() {
    ...
    const end = this.#end;
    while (start < end && isSpace(this.#buffer[start])) start++;
  ...
  trimStartChar(c) {
    let start = this.#start;
    const end = this.#end;
    while (start < end && this.#buffer[start] === c) start++;
    return new ReadonlyCharSlice(this.#buffer, start, end);
  }
  /**
   ...
   */
  trimStart(t) {
    if (t === void 0) return this.trimStartSpace();
    if (typeof t === "string") t = toCharSliceLike(t);
    if (t.length === 1) return this.trimStartChar(t.at(0) ?? -1);
  ...
  trimEndSpace() {
    ...
    let end = this.#end;
    while (start < end && isSpace(this.#buffer[end - 1])) end--;
    return new ReadonlyCharSlice(this.#buffer, start, end);
  }
  /**
  ...
  trimEndChar(char) {
    const start = this.#start;
    let end = this.#end;
    while (start < end && this.#buffer[end - 1] === char) end--;
    return new ReadonlyCharSlice(this.#buffer, start, end);
  }
  /**
   ...
   */
  trimEnd(slice) {
    if (slice === void 0) return this.trimEndSpace();
    if (typeof slice === "string") slice = toCharSliceLike(slice);
    if (slice.length === 1) return this.trimEndChar(slice.at(0) ?? -1);
  ...
  trimChar(char) {
    let start = this.#start;
    let end = this.#end;
    while (start < end && this.#buffer[start] === char) start++;
    while (start < end && this.#buffer[end - 1] === char) end--;
    return new ReadonlyCharSlice(this.#buffer, start, end);
  }
  /**
   ...
   */
  trim(slice) {
    if (slice === void 0) return this.trimSpace();
    if (typeof slice === "string") slice = toCharSliceLike(slice);
    if (slice.length === 1) return this.trimChar(slice.at(0) ?? -1);
 ...
 * @example
 * ```typescript
 * import { CharSlice } from '@neostd/slices/char-slice';
 *
...
var CharSlice = class CharSlice {
   ...
   */
  static fromString(s) {
    return new CharSlice(toCharArray(s));
  }
  /**
   * Gets the element at the specified index.
   *
  ...
  update(index, values) {
    ...
    for (let i = 0; i < values.length; i++) this.#buffer[this.#start + index + i] = values[i];
    return this;
  }
  /**
   * Iterates over the elements of the slice and calls the callback function
  ...
  replace(index, value) {
    if (index < 0 || index >= this.#end)
      throw new RangeError("Argument 'index' must be greater than or equal to 0.");
    if (typeof value === "string") value = toCharSliceLike(value);
    if (index + value.length > this.#end)
      throw new RangeError("Argument 'value' must be greater than or equal to 0.");
    for (let i = 0; i < value.length; i++) this.#buffer[this.#start + index + i] = value.at(i) ?? 0;
    return this;
  }
  /**
  ...
  captialize() {
    ...
    for (let i = this.#start + 1; i < this.#end; i++) this.#buffer[i] = toLower(this.#buffer[i]);
    return this;
  }
  /**
  ...
  findIndex(predicate) {
    for (let i = 0; i < this.length; i++)
      if (predicate(this.#buffer[this.#start + i], i, this.#buffer)) return i;
    return -1;
  }
  /**
   ...
   */
  toLower() {
    for (let j = this.#start; j < this.#end; j++) this.#buffer[j] = toLower(this.#buffer[j]);
    return this;
  }
  /**
   ...
   */
  toUpper() {
    for (let j = this.#start; j < this.#end; j++) this.#buffer[j] = toUpper(this.#buffer[j]);
    return this;
  }
  /**
  ...
  trimEndChar(c) {
    const start = this.#start;
    let end = this.#end;
    while (start < end && this.#buffer[end - 1] === c) end--;
    return new CharSlice(this.#buffer, start, end);
  }
  /**
   ...
   */
  trimEnd(t) {
    if (t === void 0) return this.trimEndSpace();
    if (t.length === 1) {
      if (typeof t === "string") t = toCharSliceLike(t);
   ...
   */
  trim(t) {
    if (t === void 0) return this.trimSpace();
    if (t.length === 1) {
      if (typeof t === "string") t = toCharSliceLike(t);
 ...
 * ```
 */
function charSlice(buffer, start, end) {
  if (typeof buffer === "string")
    return CharSlice.fromString(buffer).slice(start ?? 0, end ?? void 0);
  const uint32Buffer = buffer instanceof Uint32Array ? buffer : new Uint32Array(buffer);
  return new CharSlice(uint32Buffer, start ?? 0, end ?? uint32Buffer.length);
}
/**
 ...
 * ```
 */
function readonlyCharSlice(buffer, start, end) {
  if (typeof buffer === "string")
    return ReadonlyCharSlice.fromString(buffer).slice(start ?? 0, end ?? void 0);
  const uint32Buffer = buffer instanceof Uint32Array ? buffer : new Uint32Array(buffer);
  return new ReadonlyCharSlice(uint32Buffer, start ?? 0, end ?? uint32Buffer.length);
}
//#endregion
export { CharSlice, ReadonlyCharSlice, charSlice, readonlyCharSlice };
