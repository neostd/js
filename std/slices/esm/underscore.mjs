...
import { isDigit, isLetter, isLower, isSpace, isUpper, toLower, toUpper } from "@neostd/chars";
 ...
 * ```
 */
function underscore(slice, options) {
  options ??= {};
  ...
  for (let i = 0; i < v.length; i++) {
    ...
    if (isLetter(c)) {
      ...
      if (options.screaming) sb.appendChar(toUpper(c));
      else if (options.preserveCase) sb.appendChar(c);
      else sb.appendChar(toLower(c));
      last = c;
      continue;
    }
    if (isDigit(c)) {
