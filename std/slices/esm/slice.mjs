 ...
 * @example
 * ```typescript
 * import { Slice, ReadOnlySlice } from '@neostd/slices';
 *
 ...
 * @example
 * ```typescript
 * import { Slice } from '@neostd/slices';
 *
 * const arr = [1, 2, 3, 4, 5];
...
var Slice = class Slice {
  ...
  update(index, value) {
    if (index < 0 || index >= this.#length)
      throw new RangeError("Argument 'index' must be greater than or equal to 0.");
    let j = 0;
    for (let i = index; i < value.length; i++) this.#set[this.#offset + i] = value[j++];
    return this;
  }
  /**
  ...
  map(callbackfn) {
    ...
    for (let i = 0; i < length; i++) result[i] = callbackfn(set[offset + i], i, set);
  ...
  indexOf(value) {
    ...
    const offset = this.#offset;
    const length = this.#length;
    for (let i = 0; i < length; i++) if (set[offset + i] === value) return i;
    return -1;
  }
  /**
  ...
  slice(start, end) {
    ...
    if (end === void 0) end = this.#length - offset;
  ...
  find(predicate) {
    ...
    const offset = this.#offset;
    const length = this.#length;
    for (let i = 0; i < length; i++) if (predicate(set[offset + i], i, set)) return set[offset + i];
  }
  /**
  ...
  findIndex(predicate) {
    ...
    const offset = this.#offset;
    const length = this.#length;
    for (let i = 0; i < length; i++) if (predicate(set[offset + i], i, set)) return i;
    return -1;
  }
  /**
 ...
 * @example
 * ```typescript
 * import { ReadOnlySlice } from '@neostd/slices';
 *
 * const arr = [1, 2, 3, 4, 5];
 ...
 * ```
 */
var ReadOnlySlice = class ReadOnlySlice {
  #set;
  #offset;
  #length;
  /**
...
function slice(array, offset = 0, length) {
  return new Slice(array, offset, length);
}
function readOnlySlice(array, offset = 0, length) {
  return new ReadOnlySlice(array, offset, length);
}
//#endregion
export { ReadOnlySlice, Slice, readOnlySlice, slice };
