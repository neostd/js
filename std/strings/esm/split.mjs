 ...
 * ```
 */
function split(str, separator, trim = false, limit) {
  if (str instanceof Uint8Array) str = new TextDecoder().decode(str);
  if (str instanceof Uint32Array) str = String.fromCodePoint(...str);
  const result = str.split(separator, limit);
  if (trim) return result.map((x) => x.trim()).filter((x) => x.length > 0);
  return result;
}
//#endregion
export { split };
