...
A cross-runtime string formatting module providing printf-style formatting, value inspection, and ANSI code handling.
Works seamlessly with Deno, Node.js, Bun, and the browser. This module was inspired by the Go `fmt` package and
ported from Deno's @std/fmt, with modifications to enable node inspect and provide additional methods
like inspect, echo, echof, and errorf.

When I initially used this, @std/fmt didn't support node and did not use util.inspect when not running in Deno. Inspect
uses `Deno.inspect` when runing in Deno, and `util.inspect` running in node/bun and falls back to `json.parse` for
browsers.

![logo](https://raw.githubusercontent.com/neostd/js/refs/heads/dev/eng/assets/logo.png)

[![JSR](https://jsr.io/badges/@neostd/fmt)](https://jsr.io/@neostd/fmt)
[![npm version](https://badge.fury.io/js/@neostd%2Ffmt.svg)](https://badge.fury.io/js/@neostd%2Ffmt)
[![GitHub version](https://badge.fury.io/gh/neostd%2Fjs.svg)](https://badge.fury.io/gh/neostd%2Fjs)

## Documentation

Documentation is available on [jsr.io](https://jsr.io/@neostd/fmt/doc)
...
```bash
# Deno
deno add jsr:@neostd/fmt

# npm from jsr
npx jsr add @neostd/fmt

# from npmjs.org
npm install @neostd/fmt
```

## Quick Start

```typescript
import { echo, inspect, printf, sprintf, stripAnsiCode } from "@neostd/fmt";
...
```typescript
import { echo, errorf, printf, sprintf } from "@neostd/fmt";
...
The `inspect` function works cross-runtime using `Deno.inspect`, `util.inspect` (Node.js), or falls back to
`JSON.stringify`.

```typescript
import { inspect } from "@neostd/fmt";
...
```typescript
import { stripAnsiCode } from "@neostd/fmt";
...
@neostd/fmt is cross runtime alternative to `@std/fmt` for sprintf and printf functions. The `@std/fmt` module uses
`Deno.inspect` and `Deno.stdout.writeSync` for printf which is not available in bun, node, or the browser.

The `@neostd/fmt` module makes modifications to the `@std/fmt` module.

For inspect or the `/I` flag inspect uses `Deno.inspect`, `util.inspect` or falls back to `json.parse`. **A browsify
fallback may be included at a later date**.

For writing to a stream for printf, print, etc; the @neostd/process module is used which abstracts stdout for the
different runtimes.
...
sprintf converts and formats a variable number of arguments as is specified by a `format string`. In it's basic form, a
format string may just be a literal. In case arguments are meant to be formatted, a `directive` is contained in the
format string, preceded by a '%' character:

    `%<verb>`

The verb `s` indicates the directive should be replaced by the string representation of the argument in the
...
The meaning of the format string is modelled after [POSIX][1] format strings as well as well as
[Golang format strings][2]. Both contain elements specific to the respective programming language that don't apply to
JavaScript, so they can not be fully supported. Furthermore we implement some functionality that is specific to JS.
...
Numerical values for width and precision can be substituted for the `*` char, in which case the values are obtained from
the next args, e.g.:
...
The default format used by `%v` is the result of calling `toString()` on the relevant argument. If the `#` flags is
used, the result of calling `inspect()` is interpolated. In this case, the precision, if set is passed to `inspect()` as
